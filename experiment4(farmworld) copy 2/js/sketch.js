...
function setup() {
  ...
  let input = createInput("ALEUTIAN ISLANDS, AK");
  input.parent(label);
  input.input(() => {
...
function returnTileColour(i, j) {
  ...
  let wField = 4 + (XXH.h32("width seed:" + regionI + regionJ, worldSeed) % 9);
  let hField = 4 + (XXH.h32("height seed:" + regionI + regionJ, worldSeed) % 9);

  let fieldI = Math.floor(i / wField);
  let fieldJ = Math.floor(j / hField);

  let circlex =
    (XXH.h32("circleX:" + regionI + "," + regionJ, worldSeed) % 4) +
    regionI * 40;
  let circley =
    (XXH.h32("circleY:" + regionI + "," + regionJ, worldSeed) % 4) +
    regionJ * 40;
  let radius =
    20 + (XXH.h32("radius:" + regionI + "," + regionJ, worldSeed) % 10);

  let dx = i - circlex;
  let dy = j - circley;
  let insideCircle = dx * dx + dy * dy < radius * radius;
  let circleSnow = insideCircle;

  let hash = XXH.h32("Aleutian Islands, AK" + fieldI + fieldJ, worldSeed);
  let type = hash % 6;

  //this needs to be on top to make the return reachable
  let oceanNoise = noise(i * 0.02 - 100, j * 0.1 + 500);
  if (oceanNoise > 0.35) {
    return color(random([20, 25, 30]), random([41, 42, 45]), random([72, 75]));
  }

  if (type === 0) {
    return color(210, 206, 209);
  } else if (type === 1) {
    return color(255, 255, 255);
  } else if (circleSnow && type === 2) {
    return color(210, 206, 209);
  } else if (circleSnow && type === 3) {
    return color(255, 255, 255);
  } else if (circleSnow && type === 4) {
    return color(255, 255, 255);
  } else {
    return color(161, 143, 134);
  }
}
...
function p3_drawTile(i, j) {
  ...
  if (n % 2 == 1) {
    ...
    let baseColor = color(0, 80, 0); // tile color
