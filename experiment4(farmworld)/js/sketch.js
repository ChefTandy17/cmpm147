...
function setup() {
  ...
  let input = createInput("DAVIS, CA");
  input.parent(label);
  input.input(() => {
...
//to draw and return a tile color to check if its placeable
function returnTileColor(i, j) {
  //to design the spacing and the size of the road
  let roadSpacing = 40;
  let roadSize = 1;

  //to design the river using a noise functioin
  let riverX = Math.floor(noise(j * 0.01) * 50);
  let riverY = Math.floor(noise(i * 0.01) * 70);

  //to create vertical and horizontal roads
  let isVerticalRoad =
    Math.abs((i - Math.floor(roadSpacing / 2)) % roadSpacing) < roadSize;
  let isHorizontalRoad =
    Math.abs((j - Math.floor(roadSpacing / 2)) % roadSpacing) < roadSize;
  let roadFlag = isVerticalRoad || isHorizontalRoad;

  //to place the road, the river, and the fields
  if (roadFlag) {
    return color(80, 80, 80);
  } else if (Math.abs(i - riverX) <= 1 || Math.abs(j - riverY) <= 1) {
    return color(0, 80, random([245, 250, 255]));
  } else {
    ...
    let fieldI = Math.floor(i / wField);
    let fieldJ = Math.floor(j / hField);

    let hash = XXH.h32("Davis, CA" + fieldI + fieldJ, worldSeed);
    let type = hash % 5;

    if (type === 0) return color(100, 225, 152);
    else if (type === 1) return color(189, 183, 107);
    else return color(0, 80, 0);
  }
}

function p3_tileClicked(i, j) {
  //get the returned color tile
  let tileColor = returnTileColor(i, j);
  ...
  //determine if its the road or the river, if it is, don't make it interactable, else, make it.
  let isRoad = r === 80 && g === 80 && b === 80;
  let isRiver = r === 0 && g === 80 && (b === 245 || b === 250 || b === 255);
  if (!isRoad && !isRiver) {
    let key = [i, j];
...
function p3_drawTile(i, j) {
  ...
  //to create a blade of grass or wheats
  let n = clicks[[i, j]] | 0;
  if (n % 2 == 1) {
    let numGrass = 10;
    let baseColor = tileColor;

    push();
    translate(-tw / 4, -th / 4);

    for (let k = 0; k < numGrass; k++) {
      let x = random(tw);
      let grassHeight = random([10, 20, 25]) + random(-3, 3);
      let grassWidth = 1;

      //to adjust the color and the rectangle
      fill(lerpColor(baseColor, color(0), 0.2));
      noStroke();
      rect(x, 0, grassWidth, -grassHeight); //the negative is needed to prevent the height to be under the tile
    }
    pop();
  }
}
